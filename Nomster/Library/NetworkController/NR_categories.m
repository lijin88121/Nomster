...
NSString * const NRConnectionErrorDomain = @"NRConnectionErrorDomain";
...
@implementation NSDictionary (NR)
...
- (NSString *)urlQueryString {
    ...
    for (id key in self) {
        ...
        if (![key isKindOfClass:[NSString class]] || !isValueAcceptable(val)) {
            NRLogError(@"skipping bad parameter: key class: %@ key: %@; value class: %@; value: %@",
...
- (id)dictionaryFromJSONWithError:(NSError **)error {
    ...
    if (![dict isKindOfClass:[NSDictionary class]]) {
        ...
        *error =
        [NSError errorWithDomain:NRConnectionErrorDomain code:NRConnectionErrorCodeJSONResultType userInfo:userInfo];
...
- (NSString *)debugString {
    ...
    NSString *s = [[NSString alloc] initWithBytesNoCopy:debugBytes
                                            ...
                                            freeWhenDone:YES];
    if (!s) {
        NRLogError0(@"string encoding failed");
        free(debugBytes);
    }
    return s;
}
...
@implementation NSURLResponse (NR)

...
NSString * const NRConnectionActivityBegan = @"NRConnectionActivityBegan";
NSString * const NRConnectionActivityEnded = @"NRConnectionActivityEnded";

NSString* stringForRequestMethod(NRRequestMethod method) {
    switch (method) {
        case NRRequestMethodGET:   return @"GET";
        case NRRequestMethodPOST:  return @"POST";
        default:
...
- (NSURLRequest *)connection:(NSURLConnection *)connection
    ...
    NRVerbose(@"%p: willSendRequest", self);
    return request;
}
...
//    NRVerbose(@"%p: needNewBodyStream", self);
//}
...
- (BOOL)connectionShouldUseCredentialStorage:(NSURLConnection *)connection {
    ...
    NRVerbose(@"%p: connectionShouldUseCredentialStorage", self);
...
willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    ...
    NRVerbose(@"%p: willSendRequestForAuthenticationChallenge", self);
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSHTTPURLResponse *)response {
    ...
    NRVerbose(@"%p: didReceiveResponse", self);
    NRVerbose(@"Response Headers: %@", [response allHeaderFields]);
...
totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite {
    ...
    NRVerbose(@"%p: didSendBodyData", self);
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    NRVerbose(@"%p: didReceiveData", self);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    NRVerbose(@"%p: didFinishLoading", self);
...
+ (void)cancelAllConnections {
    for (NRConnection *c in [[self connections] allObjects]) {
        [c cancel];
    }
}
...
progressBlock:(NRProgressBlock)progressBlock {
    ...
    NRConnection *c = [NRConnection new];
...
#if TARGET_OS_IPHONE
    c.shouldRunInBackground = (method == NRRequestMethodPOST);
...
- (void)setMethod:(NRRequestMethod)method {
    ASSERT_UNSTARTED;
    _method = method;
}
...
- (void)cleanup {
    ...
    if (requests.count == 0) {
        [[NSNotificationCenter defaultCenter] postNotificationName:NRConnectionActivityEnded object:nil];
    }
}


- (void)cancelConnection {
    NRVerbose(@"%p: cancelConnection", self);
...
- (void)performComplete {
    NRVerbose(@"%p: performComplete", self);
    self.connection = nil;
...
- (NRConnection *)start {
    ...
    NRVerbose(@"%p: enqueue (#%lu)", self, (unsigned long)[[self.class connections] count]);
    ...
    if (connections.count == 1) {
        [[NSNotificationCenter defaultCenter] postNotificationName:NRConnectionActivityBegan object:nil];
    }
    ...
    if (![NSURLConnection canHandleRequest:urlRequest]) {
        [self failWithError:
         [NSError errorWithDomain:NRConnectionErrorDomain
                             code:0
...
- (void)didExpireInBackground {
    ...
    [self failWithError:
     [NSError errorWithDomain:NRConnectionErrorDomain
                         code:NRConnectionErrorCodeExpiredInBackgroundTask
...
- (NSString *)makeRequestString {
    ...
    if (self.method != NRRequestMethodGET || !self.parameters.count) {
...
- (NSData*)makePostBodyWithBoundary:(NSString*)boundary {
    NSAssert1(self.method == NRRequestMethodPOST, @"wrong method: %d", self.method);
    ...
    for (id key in self.parameters) {
        ...
        if (![key isKindOfClass:[NSString class]]) {
            NRLogError(@"skipping bad parameter key: key class: %@ key: %@", [key class], key);
        ...
        else if ([val isKindOfClass:[NRData class]]) {
            typeString  = [val mimeTypeString];
            valData     = [val data];
            fileName    = [val fileName];
        }
        else {
            NRLogError(@"skipping bad POST parameter value: key: %@; value class: %@; value: %@",
                        key, [val class], val);
...
void logParameter(NRRequestMethod method, id key, id val) {
    switch (method) {
#if !NR_LOG_GET_PARAMETERS
        case NRRequestMethodGET: break;
#endif
#if !NR_LOG_POST_PARAMETERS
        case NRRequestMethodPOST: break;
#endif
        default:
            NRErr(@"  %-16s : %@", [[key description] UTF8String], val);
    }
}
...
#if NR_LOG_REQUESTS
    NRLog(@"%@: %@", stringForRequestMethod(self.method), requestString);
...
NRErr(@"- header: %-16s : %@", [k UTF8String], v); [r setValue:v forHTTPHeaderField:k];
#else
#define SET_HEADER(k, v) [r setValue:v forHTTPHeaderField:k];
    ...
    switch (self.method) {
        ...
        case NRRequestMethodGET:
            break; // no body
            
        case NRRequestMethodPOST: {
            
            // decide if this request needs to be multipart
            BOOL multipart = NO;
            for (id k in self.parameters) {
                if ([[self.parameters valueForKey:k] isKindOfClass:[NRData class]]) {
                    multipart = YES;
                    break;
                }
            };
            ...
            if (multipart) {
                ...
                NSString *boundary = @"--NR-POST-boundary-wXzBVZAKUhpccuA9";
        ...
        default:
            [NSException raise:@"NRConnectionException" format:@"unknown request method: %d", self.method];
    }

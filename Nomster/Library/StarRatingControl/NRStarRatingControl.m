...
@implementation NRStarRatingControl
...
- (void)setupView {
	...
	currentIdx = -1;
	star = [[UIImage imageNamed:@"star_white"] retain];
	highlightedStar = [[UIImage imageNamed:@"star_yellow"] retain];
    
	for (int i=0; i<numberOfStars; i++) {
		NRStarView *v = [[NRStarView alloc] initWithDefault:self.star highlighted:self.highlightedStar position:i allowFractions:isFractionalRatingEnabled];
		[self addSubview:v];
		[v release];
	}
}
...
- (void)layoutSubviews {
	for (int i=0; i < numberOfStars; i++) {
		[(NRStarView*)[self subViewWithTag:i] centerIn:self.frame with:numberOfStars];
	}
}
...
- (void)setStar:(UIImage*)defaultStarImage highlightedStar:(UIImage*)highlightedStarImage atIndex:(int)index {
    NRStarView *selectedStar = (NRStarView*)[self subViewWithTag:index];

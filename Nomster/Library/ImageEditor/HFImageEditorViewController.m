...
static const CGFloat kPreviewImageSize = 512;
...
- (UIImage *)previewImage
{
    if(_previewImage == nil && _sourceImage != nil) {
        if(self.sourceImage.size.height > kMaxUIImageSize || self.sourceImage.size.width > kMaxUIImageSize) {
            ...
            _previewImage = [[self scaledImage:self.sourceImage  toSize:size withQuality:kCGInterpolationHigh] retain];
        } else {
...
{
    _filter = filter;
    [ZAActivityBar showWithStatus: @"Applying Filter..."];
    NSString* selectorName = [NSString stringWithFormat: @"%@Filter", filter.lowercaseString];
    CGSize size = self.previewImage.size;
    
    NSLog(@"ImageSize - %@", NSStringFromCGSize(size));
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        UIImage* filteredImage = [self.previewImage performSelectorSafely: NSSelectorFromString(selectorName)];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (UIImage*) fitToSize: (UIImage*) image
{
    UIImage* imageRet = nil;
    if(image.size.height > kMaxUIImageSize || image.size.width > kMaxUIImageSize) {
        ...
        imageRet = [[self scaledImage:image  toSize:size withQuality: kCGInterpolationHigh] retain];
    } else {
        imageRet = [image retain];
    }
    return imageRet;
}
...
- (IBAction)doneAction:(id)sender
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage* finalImage = self.sourceImage;
//        if (_filter != nil) {
//            finalImage = self.imageView.image;
//        }
        CGImageRef resultRef = [self newTransformedImage:self.imageView.transform
                                        sourceImage:finalImage.CGImage
                                         sourceSize:finalImage.size
                                  sourceOrientation:finalImage.imageOrientation
                                        outputWidth:self.outputWidth ? self.outputWidth : finalImage.size.width
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            transform = [self fitToSize: transform];
            if (_filter) {
                NSString* selectorName = [NSString stringWithFormat: @"%@Filter", _filter.lowercaseString];
                transform = [transform performSelectorSafely: NSSelectorFromString(selectorName)];
            }
